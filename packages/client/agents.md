...
**What resets the failure counter, and what escapes it.** The counter resets on any sign of a working server — a `server-intent` that parsed, or a payload that committed — not when a connection returns: a stream only ever ends by being dropped, so resetting on return would count every healthy, server-recycled connection as a failure. Waiting for a *commit* is not enough either, because a reconnect whose basis is already current is answered with the `none` intent and commits nothing, which would expire an environment whose skills never change. A non-catastrophic `goodbye` is exempt from the counter, but only on a connection that had reached a working server, which is what `reachedServer` tracks per attempt: a server that says goodbye having sent no intent delivered nothing, and exempting it would let it reconnect without limit and without any of it reaching `diagnostics` or `failed`. Keep both halves — the reset and the `reachedServer` qualifier — or one of those two failures comes back.

**HTTP 400 is recoverable exactly once.** The `basis` selector and the `If-None-Match` etag are the only client state in the request, and a selector the server no longer accepts is the one rejection the adapter can act on — so a 400 arrives as `StaleRequestStateError`, the state is dropped, and a full transfer is requested. A 400 for a request carrying neither is the request itself being refused, and is fatal. Do not make 400 unconditionally fatal again: that strands delivery for the process lifetime on a stale selector. 405, 406, 414 and 501 stay fatal, since none of them is about client state.
